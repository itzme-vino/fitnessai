...
import com.app.gateway.user.RegisterRequest;
import com.app.gateway.user.UserService;
            ...
            return userService.validateUser(userId)
                    .flatMap(exist -> {
                        if(!exist)
                        {
                            ...
                            {
                                log.info("User registering: {}", registerRequest);
                        ...
                        {
                            log.info("User exists: {}, skipping sync", finalUserId);
                            return Mono.empty();
                        }
                    })
                    .then(Mono.defer(()-> {
